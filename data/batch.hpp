...
public:
    ...
    explicit Batch(const std::size_t batchNum = 0)
        : m_mem(batchNum)
        , m_batchNum(batchNum)
    {
    }
    ...
    {
        return m_mem.rawMemory()[batchId];
    }

    [[nodiscard]] bool operator==(const Batch& rhs) const
    {
        return m_mem == rhs.m_mem && m_batchNum == rhs.m_batchNum;
    }
...
struct LowerAccessImpl<Batch<Element, Device, CategoryTags::Scalar>> {
    explicit LowerAccessImpl(Batch<Element, Device, CategoryTags::Scalar> data)
        : m_data(std::move(data))
    {
    }

    [[nodiscard]] auto mutableRawMemory()
    {
        return m_data.m_mem.rawMemory();
    }

    [[nodiscard]] const auto rawMemory() const
    {
        return m_data.m_mem.rawMemory();
    }
...
public:
    ...
    [[nodiscard]] std::size_t batchNum() const
    {
        return m_batchNum;
    }
    ...
    {
        assert(availableForWrite());
        (m_mem.rawMemory())[batchId * m_rawMatrixSize
            + rowId * m_rowLen
            + colId]
            = value;
    }
    ...
    [[nodiscard]] const auto operator[](const std::size_t batchId) const
    {
        ...
        return Matrix<ElementType, DeviceType>(m_mem.sharedPtr(), pos,
            m_rowNum, m_colNum, m_rowLen);
    }

    [[nodiscard]] Batch subMatrix(const std::size_t rowBeg, const std::size_t rowEnd,
    ...
    {
        return (m_mem == rhs.m_mem) && (m_rowNum == rhs.m_rowNum) && (m_colNum == rhs.m_colNum) && (m_batchNum == rhs.m_batchNum) && (m_rowLen == rhs.m_rowLen) && (m_rawMatrixSize == rhs.m_rawMatrixSize);
    }
...
struct LowerAccessImpl<Batch<Element, Device, CategoryTags::Matrix>> {
    explicit LowerAccessImpl(Batch<Element, Device, CategoryTags::Matrix> data)
        : m_data(std::move(data))
    {
    }
    ...
    [[nodiscard]] std::size_t RawMatrixSize() const
    {
