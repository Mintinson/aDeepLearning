...
struct SeparateParameters_ {
    ...
    template <typename... TS,
              ...
              typename... TO,
              typename TLayerTag,
              template <typename> class TLayer,
              typename... T>
    struct SeparateHelper<SubLayerContainer<TS...>,
                          InterConnectContainer<TIt...>,
                          InConnectContainer<TI...>,
                          OutConnectContainer<TO...>,
                          SubLayer<TLayerTag, TLayer>,
                          T...> {
    ...
    template <typename... TS,
              ...
              typename... TI,
              typename... TO,
              typename TOutLayerTag,
              typename TOutName,
              typename TInLayerTag,
              typename TInName,
              typename... T>
    struct SeparateHelper<SubLayerContainer<TS...>,
                          InterConnectContainer<TIt...>,
                          InConnectContainer<TI...>,
                          OutConnectContainer<TO...>,
                          InternalConnect<TOutLayerTag, TOutName, TInLayerTag, TInName>,
                          T...> {
        using Res =
            SeparateHelper<SubLayerContainer<TS...>,
    ...
    template <typename... TS,
              ...
              typename TInName,
              typename TInLayerTag,
              typename TInLayerName,
              typename... T>
    struct SeparateHelper<SubLayerContainer<TS...>,
                          InterConnectContainer<TIt...>,
                          InConnectContainer<TI...>,
                          OutConnectContainer<TO...>,
                          InConnect<TInName, TInLayerTag, TInLayerName>,
                          T...> {
    ...
    struct SeparateHelper<SubLayerContainer<TS...>,
                          ...
                          InConnectContainer<TI...>,
                          OutConnectContainer<TO...>,
                          OutConnect<TOutLayerTag, TOutLayerName, TOutName>,
                          T...> {
        using Res = SeparateHelper<SubLayerContainer<TS...>,
                                   InterConnectContainer<TIt...>,
                                   InConnectContainer<TI...>,
    ...
    using tmp = SeparateHelper<SubLayerContainer<>,
...
struct TagExistInLayerComps {
    ...
    template <typename TOutLayerTag, typename TOutName, typename TInLayerTag, typename TInName, typename... T>
...
struct InputConnectCheck<InConnectContainer<TInputConnects...>> {
    ...
    struct UniqueConnectCheckHelper<TInLayerTag, TInLayerName, TCur, T...> {
        ...
        static constexpr bool tmp3 = tmp1 || tmp2;
        static constexpr bool value = AndValue<tmp3, UniqueConnectCheckHelper<TInLayerTag, TInLayerName, T...>>;
    };
...
template <typename TInterArray, typename TInArray, typename TOutArray, typename TSublayerArray>
struct SublayerTagInOtherArrays;

template <typename... TInterElems, typename... TInElems, typename... TOutElems, typename... TSublayerElems>
...
struct UsefulInternalPostLayer<InterConnectContainer<TInterElems...>, OutConnectContainer<TOutElems...>> {
    template <typename... T>
    struct imp {
        static constexpr bool value = true;
    };
...
struct InternalLayerPrune<InterConnectContainer<T1...>,
    ...
    struct put<false, TDummy> {
        ...
        using NewTagContainer =
            std::conditional_t<CheckTagInTTags, TagContainer<TTags...>, TagContainer<TTags..., CheckTag>>;
...
template <typename TInputSubLayers, typename TOrderedSublayers, typename TUnorderedSublayers, typename TPostTags>
...
struct SeparateByPostTag<SubLayerContainer<TCur, TSI...>,
    ...
    using tmpOrdered = std::conditional_t<IsExist, SubLayerContainer<TSO..., TCur>, SubLayerContainer<TSO...>>;
    using tmpUnordered = std::conditional_t<IsExist, SubLayerContainer<TSN...>, SubLayerContainer<TSN..., TCur>>;

    using NextStep =
        SeparateByPostTag<SubLayerContainer<TSI...>, tmpOrdered, tmpUnordered, TagContainer<TPostTagElems...>>;
...
template <typename TOrderedSublayers, typename TUnorderedSublayers, typename TCheckInternals>
struct MainLoop {
...
struct MainLoop<SubLayerContainer<TSO...>, SubLayerContainer<TSN...>, InterConnectContainer<TIC, TI...>> {
    using CurInter = InterConnectContainer<TIC, TI...>;

    using NewInter = typename InternalLayerPrune<InterConnectContainer<>, CurInter, TagContainer<>, TIC, TI...>::type;
    using PostTags =
        typename InternalLayerPrune<InterConnectContainer<>, CurInter, TagContainer<>, TIC, TI...>::PostTags;
    static_assert((ArraySize<NewInter> < ArraySize<CurInter>), "Cycle exist in the compose layer");

    using SeparateByTagFun =
        SeparateByPostTag<SubLayerContainer<TSN...>, SubLayerContainer<TSO...>, SubLayerContainer<>, PostTags>;
...
struct TopologicalOrdering_<SubLayerContainer<TSubLayerElems...>, InterConnectContainer<TInterElems...>> {
    ...
    struct SublayerPreprocess_<SubLayerContainer<TSO...>, SubLayerContainer<TSN...>, TCur, T...> {
        using CurTag = typename TCur::Tag;
        static constexpr bool inInter = TagExistInLayerComps<CurTag, TInterElems...>::value;

        using NewOrdered = std::conditional_t<inInter, SubLayerContainer<TSO...>, SubLayerContainer<TSO..., TCur>>;
        using NewUnordered = std::conditional_t<inInter, SubLayerContainer<TSN..., TCur>, SubLayerContainer<TSN...>>;
    ...
    using SublayerPreRes = SublayerPreprocess_<SubLayerContainer<>, SubLayerContainer<>, TSubLayerElems...>;
    ...
    using MainLoopFun =
        NSTPO::MainLoop<OrderedAfterPreprocess, UnorderedAfterPreprocess, InterConnectContainer<TInterElems...>>;
...
struct UpdateByTag<true, TTag, SublayerPolicyContainer<TInstRes...>, SublayerPolicyContainer<TCur, TInstRemain...>> {
    ...
    using NewPolicy =
        typename std::conditional_t<checkOK, ChangePolicy_<PFeedbackOutput, OriPolicy>, Identity_<OriPolicy>>::type;
...
struct UpdateBySourceLayer<TTag, InterConnectContainer<TI, TIRemain...>, TInstCont> {
    ...
    using tmp = typename UpdateByTag<isOutTag, InTag, SublayerPolicyContainer<>, TInstCont>::type;
...
struct FeedbackOutSet<SublayerPolicyContainer<TInstElements...>, InterConnects> {
    ...
    struct imp<SublayerPolicyContainer<TProcessedInst...>, SublayerPolicyContainer<TCur, TInsts...>> {
        ...
        using tmp2 = typename std::conditional_t<
            isUpdate,
            NSFeedbackOutSet::UpdateBySourceLayer<Tag, InterConnects, SublayerPolicyContainer<TInsts...>>,
...
struct Instantiation<std::tuple<TInsts...>, SublayerPolicyContainer<TCur, TSublayers...>> {
    using Tag = typename TCur::Tag;
...
struct InternalResult_<VarTypeDict<TProcessed...>, std::tuple<TCur, TRemain...>> {
    using type = typename InternalResult_<VarTypeDict<TProcessed..., typename TCur::Tag>, std::tuple<TRemain...>>::type;
};
...
template <size_t N,
          typename TInputConnects,
          typename TOutputConnects,
          typename TInnerConnects,
          typename TSublayerMap,
          typename SublayerTuple,
          typename TInput,
          typename TInternal,
          typename TOutput>
auto FeedForwardFun(SublayerTuple& sublayers, const TInput& input, TInternal&& internal, TOutput&& output) {
...
template <size_t N,
          ...
          typename TSublayerMap,
          typename TSublayerTuple,
          typename TGrad,
          typename TInternal,
          typename TInput>
auto FeedBackwardFun(TSublayerTuple& sublayers, const TGrad& grad, TInternal&& internal, TInput&& res)
...
template <size_t N,
          typename TInitPolicies,
          typename TSublayerInfo,
...
private:
    ...
    constexpr static size_t MapTag2ID() {
        ...
        } else if constexpr (std::is_same<typename std::tuple_element<N, TSublayerTuple>::type::Tag, TTag>::value) {
            return N;
        } else {
...
struct ComposeTopology {
    ...
    static_assert(
        (NSComposeKernel::SublayerTagInOtherArrays<InterConnects, InputConnects, OutputConnects, SubLayers>::value),
    ...
    using Instances =
        typename NSComposeKernel::SublayerInstantiation<TPolicyCont, TopologicalOrdering, InterConnects>::type;
};
...
public:
    static auto CreateSubLayers() { return NSComposeKernel::SublayerArrayMaker<TInstContainer>(); }

public:
    ComposeKernel(SublayerArray p_sublayers) : sublayers(std::move(p_sublayers)) {}

public:
    template <typename TInitializer, typename TBuffer, typename TInitPolicies = typename TInitializer::PolicyCont>
    ...
    auto FeedForward(const TIn& p_in) {
        ...
        return FeedForwardFun<0, typename TKernelTopo::InputConnects, typename TKernelTopo::OutputConnects,
                              typename TKernelTopo::InterConnects, TInstContainer>(
    ...
    auto FeedBackward(const TGrad& p_grad) {
        using namespace NSComposeKernel;
        using InternalResType = NSComposeKernel::InternalResult<TInstContainer>;
        return FeedBackwardFun<ArraySize<TInstContainer>, typename TKernelTopo::InputConnects,
                               typename TKernelTopo::InterConnects, typename TKernelTopo::OutputConnects,
                               TInstContainer>(sublayers, p_grad, InternalResType::Create(), InputType::Create());
    }
    ...
    void NeutralInvariant() { NSComposeKernel::NeutralInvariant<0>(sublayers); }
