...
namespace details {
    ...
    template <typename NewType, size_t TypeIndex, size_t ModifiedNum,
        typename ProcessedTypes, typename... RemainTypes>
    struct NewTupleType;

    template <
        typename NewType,
        size_t TypeIndex, size_t ModifiedNum,
        template <typename...> class TypeContainer,
        typename... ModifiedTypes,
        typename CurType,
        typename... RemainTypes>
    struct NewTupleType<NewType, TypeIndex, ModifiedNum,
        TypeContainer<ModifiedTypes...>, CurType, RemainTypes...> {
        using type = typename NewTupleType<NewType, TypeIndex, ModifiedNum + 1,
    ...
    template <
        ...
        typename CurType,
        typename... RemainTypes>
    struct NewTupleType<NewType, TypeIndex, TypeIndex,
        TypeContainer<ModifiedTypes...>, CurType, RemainTypes...> {
    ...
    template <
        ...
        typename... RemainTypes>
    using NewTupleType_t = typename NewTupleType<NewType,
        TypeIndex, 0, TypeContainer, RemainTypes...>::type;
} // namespace metann
...
struct VarTypeDictTuple {
    ...
    public:
        ...
        explicit Values(std::tuple<ValuesTypes...>&& input) noexcept
            : m_tuple(std::move(input))
        {
        }
