...
public:
    ...
    auto feedForward(const InType& input)
    {
        ...
        if constexpr (isFeedbackOutput) {
            auto dynamic = make_dynamic(tmp);
            m_data.push(dynamic);
            return LayerIO::create().template set<LayerIO>(dynamic);
        } else {
    ...
    auto feedBackward(const GradType& grad)
    {
        if constexpr (isFeedbackOutput) {
            ...
            auto prevGrad = grad.template get<LayerIO>();
            auto& curRes = m_data.top();
            auto res = LayerIO::create().template set<LayerIO>(softmax_derivative(prevGrad, curRes));
            m_data.pop();
            return res;
        } else {
