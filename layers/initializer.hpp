...
public:
    ...
    explicit ParamInitializer(Fillers&& filler)
        : m_fillers { std::forward<Fillers>(filler) }
    {
    }
    ...
    {
        auto newFiller = std::move(m_fillers).template set<Tag, Val>(
            std::forward<Val>(val));
        using NewFillerType = std::remove_cvref_t<decltype(newFiller)>;
        return ParamInitializer<Element, PolicyCont, NewFillerType> {
            std::move(newFiller)
        };
    }
