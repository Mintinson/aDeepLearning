...
int main() {
    ...
    std::cout
        << typeid(metann::RemoveTypeFromContainer_t<int, std::tuple<double, int, float, int, long, double>>).name()
        << std::endl;
    ...
    std::cout
        << typeid(metann::Filter_t<true, std::is_integral, std::tuple, double, float, uint8_t, int, std::string>).name()
        << std::endl;
    std::cout
        << typeid(metann::TransformTo_t<std::tuple<std::vector<int>, std::vector<double>>, std::tuple, Oper>).name()
        << std::endl;
    std::cout << metann::Key2IDFromContainer_v<int, std::tuple<char, double, int>> << std::endl;
    std::cout << typeid(metann::ContainerTail_t<std::tuple<int, double, float, char>>).name() << std::endl;
}
