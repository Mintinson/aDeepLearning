...
public:
    ...
    bool operator==(const UnaryOperator& val) const
    {
...
namespace eval {
    ...
    struct UnaryCalculator {
        ...
        {
            static_assert(
                std::is_same_v<CaseTail, OperSeqContainer<>>,
            ...
            using UnitType = typename UnitWrapper<OperTag>::template EvalUnit<
                decltype(handle), ElementType, DeviceType, CateType>;
    ...
    struct UnitWrapper<UnaryOperTags::Transpose> {
        ...
        class EvalUnit<OperHandle, Element, CPU, CategoryTags::Matrix>
        ...
        template <typename OperHandle, typename Element>
        class EvalUnit<OperHandle, Element, CPU, CategoryTags::BatchMatrix>
            : public BaseEvalUnit<CPU> {
        public:
            using ElementType = Element;
            using DeviceType = CPU;

            EvalUnit(OperHandle oper,
...
struct eval::UnitWrapper<UnaryOperTags::Collapse> {
    ...
    template <typename OperHandle, typename Element>
    class EvalUnit<OperHandle, Element, CPU>
        : public BaseEvalUnit<CPU> {
    public:
        using ElementType = Element;
        using DeviceType = CPU;
    ...
    struct UnaryCalculator<UnaryOperTags::Collapse> {
        ...
        static void evalRegister(EvalRes& evalRes, const Operand& oper)
        {
            ...
            auto handle
                = oper.evalRegister();
            using UnitType = typename UnitWrapper<OperTag>::template EvalUnit<
                decltype(handle), ElementType, DeviceType>;
            using GroupType = TrivialEvalGroup<UnitType>;
...
auto abs(MatType&& mat)
{
    ...
    return details::UnaryOperatorForMatrix<
        UnaryOperTags::Abs,
        MatType>::eval(std::forward<MatType>(mat));
}

template <>
struct OperSeq<UnaryOperTags::Abs> {
    using type = OperSeqContainer<eval::UnaryCalculator<
        eval::TrivialUnaryOperatorTag<decltype([](auto x) {
            return std::abs(x);
        })>>>;
};
...
struct OperSeq<UnaryOperTags::Sign> {
    using type = OperSeqContainer<eval::UnaryCalculator<
        eval::TrivialUnaryOperatorTag<decltype([]<typename T0>(T0&& x) {
            return (x > 0)
                ? static_cast<std::remove_reference_t<T0>>(1)
                : static_cast<std::remove_reference_t<T0>>(-1);
        })>>>;
};
...
struct eval::UnitWrapper<UnaryOperTags::VecSoftmax> {
    template <typename OperHandle, typename Element,
        DeviceConcept Device, CategoryConcept Cate>
    class EvalUnit;
