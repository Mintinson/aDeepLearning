...
public:
    using type = std::conditional_t<std::is_same_v<U, T>, ResType, ConcatContainer_t<Container<U>, ResType>>;
};
...
template <bool keepTrue, template <typename> class Pred, template <typename...> class Container, typename... Args>
struct Filter {
    using type = Container<>;
};

template <bool keepTrue,
...
public:
    using type = std::conditional_t<Pred<Cur>::value == keepTrue, ConcatContainer_t<Container<Cur>, ResType>, ResType>;
};
