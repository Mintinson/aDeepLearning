...
public:
    ...
    bool operator==(const BinaryOperator& val) const { return (m_data1 == val.m_data1) && (m_data2 == val.m_data2); }
...
concept ElemOperrable = (IsMatrix_v<T> && IsMatrix_v<U>) || Commutative_v<T, U, IsMatrix, IsScalar> ||
                        Commutative_v<T, U, IsMatrix, IsBatchMatrix> || Commutative_v<T, U, IsScalar, IsBatchMatrix> ||
...
public:
    ...
    static auto eval(P1&& m, P2&& s) {
        ...
        auto tempMatrix = make_trivial_matrix<ElementType, DeviceType>(m.rowNum(), m.colNum(), std::forward<P2>(s));
    ...
    static auto eval(P1&& s, P2&& m) {
        if constexpr (IsInPack_v<OperTag, BinaryOperTags::Sub, BinaryOperTags::Div>) {
            ...
            auto tempMatrix = make_trivial_matrix<ElementType, DeviceType>(m.rowNum(), m.colNum(), std::forward<P1>(s));
            ...
            return OperatorTrivial<OperTag, P2, P1>::template eval<U, T>(std::forward<P2>(m), std::forward<P1>(s));
        }
    }
    ...
    static auto eval(P1&& m1, P2&& m2) {
        if constexpr (IsInPack_v<OperTag, BinaryOperTags::Sub, BinaryOperTags::Div>) {
            ...
            return OperatorTrivial<OperTag, P2, P1>::template eval<U, T>(std::forward<P2>(m2), std::forward<P1>(m1));
        }
    }
...
struct UnitWrapper<TrivialBinaryOperatorTag<Operator>> {
    ...
    class EvalUnit<OperHandle1, OperHandle2, Element, CPU, CategoryTags::Matrix> : public BaseEvalUnit<CPU> {
    public:
        using ElementType = Element;
        using DeviceType = CPU;

        EvalUnit(OperHandle1 op1, OperHandle2 op2, EvalHandle<Matrix<Element, CPU>> evalOutput)
            : m_oper1(std::move(op1))
            , m_oper2(std::move(op2))
    ...
    template <typename OperHandle1, typename OperHandle2, typename Element>
    class EvalUnit<OperHandle1, OperHandle2, Element, CPU, CategoryTags::BatchMatrix> : public BaseEvalUnit<CPU> {
    public:
        using ElementType = Element;
        using DeviceType = CPU;

        EvalUnit(OperHandle1 op1,
                 OperHandle2 op2,
...
struct BinaryCalculator {
    ...
    static void evalRegister(EvalRes& evalRes, const Operator1& oper1, const Operator2& oper2) {
        ...
        using UnitType =
            typename UnitWrapper<OperatorEvalTag>::template EvalUnit<decltype(handle1), decltype(handle2), ElementType,
...
auto operator+(P1&& m1, P2&& m2) {
    ...
    return details::OperatorTrivial<BinaryOperTags::Add, P1, P2>::template eval<Cate1, Cate2>(std::forward<P1>(m1),
                                                                                              std::forward<P2>(m2));
}
...
auto operator-(P1&& m1, P2&& m2) {
    ...
    return details::OperatorTrivial<BinaryOperTags::Sub, P1, P2>::template eval<Cate1, Cate2>(std::forward<P1>(m1),
                                                                                              std::forward<P2>(m2));
}
...
auto operator*(P1&& m1, P2&& m2) {
    ...
    return details::OperatorTrivial<BinaryOperTags::Mul, P1, P2>::template eval<Cate1, Cate2>(std::forward<P1>(m1),
                                                                                              std::forward<P2>(m2));
}
...
auto operator/(P1&& m1, P2&& m2) {
    ...
    return details::OperatorTrivial<BinaryOperTags::Div, P1, P2>::template eval<Cate1, Cate2>(std::forward<P1>(m1),
                                                                                              std::forward<P2>(m2));
}

/*************************************** Dot operator *******************************************/
template <typename T, typename U>
concept Dottable = (IsMatrix_v<T> && IsMatrix_v<U>) || Commutative_v<T, U, IsMatrix, IsBatchMatrix> ||
                   (IsBatchMatrix_v<T> && IsBatchMatrix_v<U>);
...
public:
    ...
    OperOrganizer(const DataType1& m1, const DataType2& m2) : m_rowNum(m1.rowNum())
                                                            , m_colNum(m2.colNum()) {
...
auto neg_log_likelihood(P1&& m1, P2&& m2) {
    ...
    // return details::BinaryOperatorForSameDataType<BinaryOperTags::NegativeLogLikelihood, P1,
    //                                               P2>::template eval<Cate1, Cate2>(std::forward<P1>(m1),
    //                                                                                std::forward<P2>(m2));
    return details::BinaryOperatorForSameDataType<BinaryOperTags::NegativeLogLikelihood, P1, P2>::eval(
...
struct eval::UnitWrapper<BinaryOperTags::NegativeLogLikelihood> {
    ...
    template <typename OperHandle1, typename OperHandle2, typename Element>
    class EvalUnit<OperHandle1, OperHandle2, Element, CPU, CategoryTags::Scalar> : public BaseEvalUnit<CPU> {
    public:
        using ElementType = Element;
        using DeviceType = CPU;

        EvalUnit(OperHandle1 op1, OperHandle2 op2, EvalHandle<Scalar<Element, CPU>> evalOutput)
            : m_oper1(std::move(op1))
            , m_oper2(std::move(op2))
            , m_evalOutput(std::move(evalOutput)) {}
    ...
    template <typename OperHandle1, typename OperHandle2, typename Element>
    class EvalUnit<OperHandle1, OperHandle2, Element, CPU, CategoryTags::BatchScalar> : public BaseEvalUnit<CPU> {
    public:
        using ElementType = Element;
        using DeviceType = CPU;
...
auto softmax_derivative(P1&& m1, P2&& m2) {
    ...
    return details::BinaryOperatorForSameDataType<BinaryOperTags::VecSoftmaxDerivative, P1, P2>::eval(
        std::forward<P1>(m1), std::forward<P2>(m2));
}
...
struct eval::UnitWrapper<BinaryOperTags::VecSoftmaxDerivative> {
    ...
    public:
        ...
        void eval() override {
            ...
            using EvalUnit = UnitWrapper<BinaryOperTags::Dot>::EvalUnit<decltype(m_oper1), decltype(tempHandle),
    ...
    public:
        ...
        void eval() override {
            ...
            for (std::size_t curBatch = 0; curBatch < batchNum; ++curBatch) {
                for (std::size_t i = 0; i < colNum; ++i) {
                    for (std::size_t j = 0; j < colNum; ++j) {
                        tmp.setValue(curBatch, i, j, -1 * pV2[curBatch](0, i) * pV2[curBatch](0, j));
                    }
            ...
            auto tempHandle = tmp.evalRegister();
            using EvalUnit =
                UnitWrapper<BinaryOperTags::Dot>::EvalUnit<decltype(m_oper1), decltype(m_oper2), ElementType,
                                                           DeviceType, CategoryTags::BatchMatrix>;
            using GroupType = TrivialEvalGroup<EvalUnit>;
            const void* dataPtr = m_evalOutput.dataPtr();
            auto depVec = {m_oper1.dataPtr(), tempHandle.dataPtr()};
            EvalUnit unit(m_oper1, std::move(tempHandle), std::move(m_evalOutput));
...
auto sigmoid_derivative(P1&& m1, P2&& m2) {
    ...
    return details::BinaryOperatorForSameDataType<BinaryOperTags::SigmoidDerivative, P1, P2>::eval(
        std::forward<P1>(m1), std::forward<P2>(m2));
}

// template <DataConcept P1, DataConcept P2>
// auto vec_sigmoid_derivative(P1&& m1, P2&& m2) {
//     using Cate1 = DataCategory_t<P1>;
//     using Cate2 = DataCategory_t<P2>;
...
auto tanh_derivative(P1&& m1, P2&& m2) {
    ...
    return details::BinaryOperatorForSameDataType<BinaryOperTags::TanhDerivative, P1, P2>::eval(std::forward<P1>(m1),
                                                                                                std::forward<P2>(m2));
}
