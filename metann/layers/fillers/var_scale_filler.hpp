...
public:
    VarScaleFiller(double factor = 1, unsigned seed = std::random_device{}()) : m_factor(factor), m_engine(seed) {}
...
template <typename PolicyCont = PolicyContainer<>>
class MSRAFiller : public VarScaleFiller<ChangePolicy_t<VarScaleFanIn, ChangePolicy_t<NormVarScale, PolicyCont>>> {
