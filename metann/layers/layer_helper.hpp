...
template <bool activate,
          bool batchMode,
          typename Element,
          DeviceConcept DeviceType,
          CategoryConcept CateTypeSingle,
          CategoryConcept CateTypeBatch>
struct LayerInternalBuf {
    using type = std::conditional_t<batchMode,
                                    LayerInternalBufType_t<Element, DeviceType, CateTypeBatch>,
                                    LayerInternalBufType_t<Element, DeviceType, CateTypeSingle>>;
};
...
template <bool activate,
          ...
          typename DeviceType,
          typename CateTypeSingle,
          typename CateTypeBatch>
using LayerInternalBuf_t =
    typename LayerInternalBuf<activate, batchMode, ElementType, DeviceType, CateTypeSingle, CateTypeBatch>::type;
}  // namespace details
...
struct FeedbackOut {
    ...
    using InternalType = LayerInternalBufType_t<ElementType, DeviceType, CategoryTags::Matrix>;
...
void matrix_grad_collect(const Weight& weight, Grad& grad, GradCollector& col) {
    while (!grad.empty()) {
        auto g = grad.top();
        grad.pop();
