...
namespace metann
{
    ...
    class DynamicWrapper
        : public DynamicCategory<typename BaseData::ElementType,
        typename BaseData::DeviceType,
        DataCategory_t<BaseData>>
    {
        using BaseClass = DynamicCategory<typename BaseData::ElementType,
            typename BaseData::DeviceType,
            DataCategory_t<BaseData>>;

    public:
        DynamicWrapper(BaseData data)
            : BaseClass(data)
    ...
    public:
        ...
        DynamicData(std::shared_ptr<DynamicWrapper<OriginalData>> data)
    ...
    public:
        ...
        template <typename OriginalData>
        explicit DynamicData(std::shared_ptr<DynamicWrapper<OriginalData>> data)
            : m_baseData(std::move(data))
        {
        }
        bool operator==(const DynamicData& val) const
    ...
    auto make_dynamic(DataType&& data)
    {
        if constexpr (IsDynamic_v<DataType>) {
            ...
            return DynamicData<
                typename RawDataType::ElementType,
                typename RawDataType::DeviceType,
