...
struct TagExistInLayerComps {
    ...
    template <typename Tag, typename OutLayerTag, typename OutName, typename InLayerTag, typename InName>
...
struct InternalConnectChecker<InterConnectContainer<Sublayers...>> {
    constexpr static bool NoSelfCycle =
        !std::disjunction_v<std::is_same<typename Sublayers::OutTag, typename Sublayers::InTag>...>;

    template <typename Type1, typename Type2>
    struct UniqueSourceHelper {
        constexpr static bool value1 = std::is_same_v<typename Type1::InTag, typename Type2::InTag>;
        constexpr static bool value2 = std::is_same_v<typename Type1::InName, typename Type2::InName>;
...
struct InputConnectChecker<InConnectContainer<Sublayers...>> {
    template <typename Type1, typename Type2>
    struct UniqueSourceHelper {
        constexpr static bool value1 = std::is_same_v<typename Type1::InLayerTag, typename Type2::InLayerTag>;
        constexpr static bool value2 = std::is_same_v<typename Type1::InLayerName, typename Type2::InLayerName>;
        constexpr static bool value = value1 && value2;
    };
...
struct OutputConnectChecker<OutConnectContainer<Sublayers...>> {
    template <typename Type1, typename Type2>
    struct UniqueSourceHelper {
        constexpr static bool value = std::is_same_v<typename Type1::OutName, typename Type2::OutName>;
    };
...
struct InternalTagInSublayer<InterConnectContainer<IntSubLayers...>, SubLayerContainer<Sublayers...>> {
    template <typename T, typename... Args>
    struct InternalTagHelper {
        using CurIn = typename T::InTag;
        using CurOut = typename T::OutTag;
        constexpr static bool value =
            TagExistInLayerComps<CurOut, Args...>::value && TagExistInLayerComps<CurIn, Args...>::value;
    };
...
struct InputTagInSubLayer<InConnectContainer<InputSubLayers...>, SubLayerContainer<Sublayers...>> {
    template <typename T, typename... Args>
...
struct OutputTagInSubLayer<OutConnectContainer<OutputSubLayers...>, SubLayerContainer<Sublayers...>> {
    template <typename T, typename... Args>
...
template <typename InterArray, typename InArray, typename OutArray, typename SublayerArray>
struct SublayerTagInOtherArrays;

template <typename... InterElems, typename... InElems, typename... OutElems, typename... SublayerElems>
...
struct UsefulInternalPostLayer<InterConnectContainer<InterElems...>, OutConnectContainer<OutElems...>> {
    template <typename Cur>
...
struct FeedbackOutChecker<true, SublayerPolicyContainer<InstElements...>> {
    ...
    struct FeedbackOutPred {
        constexpr static bool value = PolicySelect_t<FeedbackPolicy, typename SubPolicy::Policy>::isFeedbackOutput;
    };
