 ...
 * @return type: new InterConnectContainer container tahat contains the sublayers that are connected to the predecessor
 layer
 */
 ...
 */
template <typename OrderedSubLayers, typename UnOrderedSubLayers, typename PostTagsContains>
...
struct SeparateByPostTag<SubLayerContainer<OrderedElements...>,
    ...
    using SeparatedFunc = SeparateBy<SeparateHelper, SubLayerContainer, SubLayerContainer, UnOrderedElements...>;
...
struct UpdateByTag<true, TTag, SublayerPolicyContainer<TInstRes...>, SublayerPolicyContainer<TCur, TInstRemain...>> {
    ...
    using NewPolicy =
        std::conditional_t<checkOK, ChangePolicy_t<FeedbackOutputPolicy, OriPolicy>, std::type_identity_t<OriPolicy>>;
    ...
    struct imp<SublayerPolicyContainer<TProcessedInst...>, SublayerPolicyContainer<TCur, TInsts...>> {
        ...
        using tmp2 =
            typename std::conditional_t<isUpdate,
                                        UpdateBySourceLayer<Tag, InterConnects, SublayerPolicyContainer<TInsts...>>,
...
private:
    ...
    constexpr static std::size_t mapTag2ID() {
        ...
        } else if constexpr (std::is_same_v<Tag, typename std::tuple_element_t<N, std::tuple<SubLayers...>>::Tag>) {
            return N;
        } else {
...
auto input_from_in_connect(const Input& input, Res&& res) {
    if constexpr (ContainerSize_v<InputConnects> == 0) {
        ...
        if constexpr (std::is_same_v<AimTag, typename Cur::InLayerTag>) {
            ...
            auto cur = std::forward<Res>(res).template set<InLayerName>(input.template get<InName>());
 ...
 * @tparam Res Input container of the sub-layer
 */
template <typename AimTag, typename InternalConnects, typename Internal, typename Res>
auto input_from_internal_connect(const Internal& input, Res&& res) {
    if constexpr (ContainerSize_v<InternalConnects> == 0) {
        ...
        if constexpr (std::is_same_v<AimTag, typename Cur::InTag>) {
            ...
            auto cur = std::forward<Res>(res).template set<InName>(preLayer.template get<OutName>());
...
auto fill_output(const Res& curLayerRes, OutCont&& output) {
    if constexpr (ContainerSize_v<OutputConnects> == 0) {
        ...
        if constexpr (std::is_same_v<AimTag, typename Cur::OutLayerTag>) {
            ...
            return fill_output<AimTag, Tail>(curLayerRes, std::forward<OutCont>(output));
        }
    }
}
...
struct CreateGradFromOutside<AimTag, OutConnectContainer<Cur, Others...>> {
    ...
    static auto eval(const Grad& grad, Res&& res) {
        ...
        if constexpr (std::is_same_v<AimTag, typename Cur::OutLayerTag>) {
            ...
            if constexpr (std::is_same_v<std::remove_cvref_t<OrigParamType>, NullParameter>) {
