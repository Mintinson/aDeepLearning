...
namespace metann
{
    namespace details
    {
        ...
        template <typename... OrderElems, typename... UnorderedElems,
            typename InterElem, typename... InterOthers>
            ...
            InterConnectContainer<InterElem, InterOthers...>>
        {
            ...
            using NewInter = typename InternalLayerPrune<InterConnectContainer<>,
                CurInter, TagContainer<>,
                InterElem, InterOthers...>::type;
            using PostTags = typename InternalLayerPrune<InterConnectContainer<>,
                CurInter, TagContainer<>,
                InterElem, InterOthers...>::PostTags;
            static_assert(ContainerSize_v<NewInter> < ContainerSize_v<CurInter>,
                "Cycle exist in the compose layer");
            using SeparateByTagFun = SeparateByPostTag<SubLayerContainer<OrderElems...>,
                SubLayerContainer<UnorderedElems...>,
                PostTags>;
        ...
        struct SubLayerInstantiation
        {
            ...
            constexpr static bool isPlainPolicyFeedbackOut =
                PolicySelect_t<FeedbackPolicy, PlainPolicies>::isFeedbackOutput;
            ...
            using FeedbackOutUpdate = typename std::conditional_t<
                isPlainPolicyFeedbackOut,
    ...
    struct TopologicalOrdering<SubLayerContainer<SubLayerElems...>, InterConnectContainer<InterElems...>>
    {
        ...
        using MainLoopFun = details::MainLoop<OrderedAfterPreprocess,
            UnorderedAfterPreprocess,
    ...
    public:
        ...
        static_assert(details::SublayerTagInOtherArrays<InterConnects, InputConnects,
    ...
    namespace details
    {
        ...
        template <std::size_t N,
            typename InitPolicies, typename SubLayerInfo,
            typename Initializer, typename Buffer,
            typename SubLayerTuple>
        void init(Initializer& initializer, Buffer& loadBuffer,
            std::ostream* log, SubLayerTuple& sublayers)
        {
            if constexpr (N < std::tuple_size_v<SubLayerTuple>)
            {
                ...
                layer_init<typename LayerInfo::Layer, Initializer, Buffer, NewInitPolicy>(
                    *layer, initializer, loadBuffer, log);
                init<N + 1, InitPolicies, SubLayerInfo>(initializer, loadBuffer,
                    log, sublayers);
            }
        }
        ...
        template <std::size_t N,
            ...
            typename Output
        >
        auto feed_forward_func(SubLayerTuple& subLayers, const Input& input,
        ...
        template <std::size_t N,
            ...
            typename Internal,
            typename Input>
        auto feed_backward_func(SublayerTuple& subLayers, const Grad& grad,
            Internal&& internal, Input&& res)
        {
            if constexpr (N == 0)
            {
    ...
    public:
        ComposeKernel(SubLayerArray subLayers)
        ...
        {
            return details::SubLayerArrayMaker<InstContainer>();
        }
        ...
        {
            details::neutral_invariant<0>(m_subLayers);
        }
        ...
        auto feedForward(const In& input)
        {
            ...
            return details::feed_forward_func<0,
                typename KernelTopo::InputConnects,
                typename KernelTopo::OutputConnects,
                typename KernelTopo::InterConnects,
                InstContainer>(
                    m_subLayers, input,
                    InternalResType::create(), OutputType::create());
        }
        ...
        {
            using InternalResType = details::InternalResult_t<InstContainer>;
            return details::feed_backward_func<ContainerSize_v<InstContainer>,
                typename KernelTopo::InputConnects,
                typename KernelTopo::InterConnects,
                typename KernelTopo::OutputConnects,
                InstContainer>
                (m_subLayers, grad, InternalResType::create(), InputType::create());
        }

        static constexpr bool isFeedbackOutput = details::PolicySelect_t<
            FeedbackPolicy, PlainPolicies>::isFeedbackOutput;
