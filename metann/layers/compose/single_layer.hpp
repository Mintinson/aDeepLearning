...
struct TopologyHelper {
    using type = ComposeTopology<
        SubLayer<SubLayerOf<SingleLayer>::Weight, WeightLayer>,
        ...
        InternalConnect<SubLayerOf<SingleLayer>::Weight, LayerIO, SubLayerOf<SingleLayer>::Bias, LayerIO>,
        InternalConnect<SubLayerOf<SingleLayer>::Bias, LayerIO, SubLayerOf<SingleLayer>::Action, LayerIO>,
...
struct TopologyHelper<false, TActLayer> {
    using type = ComposeTopology<
        ...
        InConnect<LayerIO, SubLayerOf<SingleLayer>::Weight, LayerIO>,
        InternalConnect<SubLayerOf<SingleLayer>::Weight, LayerIO, SubLayerOf<SingleLayer>::Action, LayerIO>,
        OutConnect<SubLayerOf<SingleLayer>::Action, LayerIO, LayerIO>>;
};
...
struct KernelHelper {
    ...
    using ActType = typename ActionPicker<
        typename details::PolicySelect_t<SingleLayerPolicy, PlainPolicies>::Action>::template type<T>;
...
template <typename Policies>
class SingleLayer
    : public ComposeKernel<LayerIO, LayerIO, Policies, typename details::single_layer::KernelHelper<Policies>::type> {
    using Base =
        ComposeKernel<LayerIO, LayerIO, Policies, typename details::single_layer::KernelHelper<Policies>::type>;

public:
    SingleLayer(const std::string& name, const size_t inputLen, const size_t outputLen)
        : Base{details::single_layer::TupleCreator<details::single_layer::KernelHelper<Policies>::hasBias, Base>(
              name, inputLen, outputLen)} {}
};
}  // namespace metann
