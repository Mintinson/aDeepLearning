...
struct MinorDedup<MinorClass, CurPolicies, Policies...>
    : std::conjunction<std::negation<std::is_same<MinorClass, typename CurPolicies::MinorClass>>,
...
public:
    static constexpr bool value =
        std::conjunction_v<std::bool_constant<curCheck>,
                           std::bool_constant<MinorCheck<PolicyContainer<Policies...>>::value>>;
};
...
template <typename... FilteredPolicies, typename MajorClass, typename CurPolicy, typename... Policies>
struct MajorFilter<PolicyContainer<FilteredPolicies...>, MajorClass, CurPolicy, Policies...> {
    ...
    struct _impl<MajorClass, TDummy> {
        using type =
            typename MajorFilter<PolicyContainer<FilteredPolicies..., CurPolicy>, MajorClass, Policies...>::type;
    };
...
struct PolicySelRes<PolicyContainer<CurPolicy, Policies...>>
    : public CurPolicy
    , public PolicySelRes<PolicyContainer<Policies...>> {};
...
struct Selector<MajorClass, PolicyContainer<Policies...>> {
    ...
    static_assert(MinorCheck_v<FilterContainer>, "Minor Class set conflict!");

    using type = std::conditional_t<IsContainerEmpty_v<FilterContainer>, MajorClass, PolicySelRes<FilterContainer>>;
};
...
struct PlainPolicy<PolicyContainer<CurPolicy, Policies...>, PolicyContainer<FilteredPolicies...>> {
    using NewFiltered =
        std::conditional_t<IsSubPolicyContainer_v<CurPolicy>,
                           PlainPolicy<PolicyContainer<Policies...>, PolicyContainer<FilteredPolicies...>>,
                           PlainPolicy<PolicyContainer<Policies...>, PolicyContainer<FilteredPolicies..., CurPolicy>>>;
...
struct PolicyDeriveFil_<PolicyContainer<TCur, TProcessings...>, PolicyContainer<TProcessed...>, TCompPoliCont> {
    constexpr static bool dupe = PolicyExist_v<TCur, TCompPoliCont>;
    using TNewFiltered = std::conditional_t<dupe, PolicyContainer<TProcessed...>, PolicyContainer<TProcessed..., TCur>>;
    using type = typename PolicyDeriveFil_<PolicyContainer<TProcessings...>, TNewFiltered, TCompPoliCont>::type;
};

template <typename TCompPoliCont, typename... TProcessed>
struct PolicyDeriveFil_<PolicyContainer<>, PolicyContainer<TProcessed...>, TCompPoliCont> {
    using type = PolicyContainer<TProcessed...>;
};

template <typename TSubPolicies, typename TParentPolicies>
struct PolicyDerive {
    using TFiltered = typename PolicyDeriveFil_<TParentPolicies, PolicyContainer<>, TSubPolicies>::type;
    // template <typename T>
    // struct Pred
    // {
