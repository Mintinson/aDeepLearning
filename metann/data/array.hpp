...
struct UnitWrapper<ScalarArrayEvalTag> {
    ...
    struct EvalUnit<InputELem, Elem, CPU, CategoryTags::Scalar> : public BaseEvalUnit<CPU> {
        using ElementType = Elem;
        using DeviceType = CPU;

        EvalUnit(std::vector<InputELem> input, EvalHandle<Batch<ElementType, CPU, CategoryTags::Scalar>> output)
...
struct UnitWrapper<MatrixArrayEvalTag> {
    ...
    template <typename InputELem, typename Elem>
    struct EvalUnit<InputELem, Elem, CPU, CategoryTags::Matrix> : public BaseEvalUnit<CPU> {
        using ElementType = Elem;
        using DeviceType = CPU;

        EvalUnit(std::vector<InputELem> input, EvalHandle<Batch<ElementType, CPU, CategoryTags::Matrix>> output)
            : m_inputs(std::move(input))
            , m_output(std::move(output)) {}
...
public:
    ...
    ArrayImpl(IterType begin, IterType end) : m_buffer(new std::vector<ElementType>(begin, end)) {}
    ...
    [[nodiscard]] bool availableForWrite() const { return !m_evalBuf.isEvaluated() && m_buffer.use_count() == 1; }
    ...
    [[nodiscard]] const auto& operator[](std::size_t id) const { return (*m_buffer)[id]; }

    [[nodiscard]] auto& operator[](std::size_t id) { return (*m_buffer)[id]; }
    ...
    auto evalRegister() const {
        if (!m_evalBuf.isEvaluated()) {
            ...
            using EvalUnit = eval::UnitWrapper<eval::ScalarArrayEvalTag>::EvalUnit<OpEvalHandle, ElementType,
                                                                                   DeviceType, CategoryTags::Scalar>;
...
public:
    ...
    ArrayImpl(const std::size_t rowNum, const std::size_t colNum)
        : m_rowNum(rowNum)
        , m_colNum{colNum}
        , m_buffer{std::make_shared<std::vector<DataType>>()} {}

    template <typename IterType>
        requires requires { typename std::iterator_traits<IterType>::iterator_category; }
    ArrayImpl(IterType begin, IterType end) : m_buffer(new std::vector<DataType>(begin, end)) {
        if (!m_buffer->empty()) {
            m_rowNum = m_buffer->at(0).rowNum();
            m_colNum = m_buffer->at(0).colNum();
            for (size_t i = 1; i < m_buffer->size(); ++i) {
                if ((m_buffer->operator[](i).rowNum() != m_rowNum) || (m_buffer->operator[](i).colNum() != m_colNum)) {
    ...
    auto evalRegister() const {
        if (!m_evalBuf.isEvaluated()) {
            ...
            using EvalUnit = eval::UnitWrapper<eval::MatrixArrayEvalTag>::EvalUnit<TOpEvalHandle, ElementType,
                                                                                   DeviceType, CategoryTags::Matrix>;
            using GroupType = TrivialEvalGroup<EvalUnit>;
