...
struct UnitWrapper<DupScalarEvalTag> {
    ...
    struct EvalUnit<InHandle, Element, CPU, CategoryTags::Scalar> : public BaseEvalUnit<CPU> {
    public:
        EvalUnit(InHandle oper, size_t batchNum, EvalHandle<Batch<Element, CPU, CategoryTags::Scalar>> evalOutput)
...
struct UnitWrapper<DupMatrixEvalTag> {
    ...
    template <typename InHandle, typename Element>
    struct EvalUnit<InHandle, Element, CPU, CategoryTags::Matrix> : public BaseEvalUnit<CPU> {
    public:
        EvalUnit(InHandle oper, size_t batchNum, EvalHandle<Batch<Element, CPU, CategoryTags::Matrix>> evalOutput)
            : m_oper(std::move(oper))
            , m_batchNum(batchNum)
            , m_evalOutput(std::move(evalOutput)) {}
...
public:
    ...
    DuplicateImpl(DataType data, const std::size_t batchNum) : m_data(std::move(data)), m_batchNum(batchNum) {}
    ...
    auto evalRegister() const {
        if (!m_evalBuf.isEvaluated()) {
            ...
            using EvalUnit = eval::UnitWrapper<eval::DupScalarEvalTag>::EvalUnit<decltype(inHandle), ElementType,
            ...
            EvalPlan<DeviceType>::template registerFun<GroupType>(std::move(unit), dataPtr, {depPtr});
        }
...
public:
    ...
    auto evalRegister() const {
        if (!m_evalBuf.isEvaluated()) {
            ...
            using EvalUnit = eval::UnitWrapper<eval::DupMatrixEvalTag>::EvalUnit<decltype(inHandle), ElementType,
