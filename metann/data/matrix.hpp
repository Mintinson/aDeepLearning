...
struct Matrix<Element, CPU>  // specialization for CPU scalar
{
    ...
    [[nodiscard]] std::size_t rowLen() const { return m_rowLen; }
    ...
    // private:
    Matrix(std::shared_ptr<ElementType> mem,
...
public:
    [[nodiscard]] Matrix subMatrix(const std::size_t rowBeg,
...
public:
    ...
    auto evalRegister() const {
        ...
        if (!m_evalBuf.isEvaluated()) {
            ...
            EvalPlan<DeviceType>::template registerFun<EvalGroup>(std::move(unit), outputPtr, {});
        }
...
struct UnitWrapper<ZeroMatrixEvalTag> {
    ...
    public:
        ...
        EvalUnit(EvalHandle<Matrix<Element, DeviceType>> resBuf, const std::size_t rowNum, const std::size_t colNum)
            : m_resHandle(std::move(resBuf))
            , m_rowNum(rowNum)
            , m_colNum(colNum) {}
...
public:
    ...
    ZeroMatrix(const std::size_t rowNum, const std::size_t colNum) : m_rowNum{rowNum}, m_colNum{colNum} {}
    ...
    // TODO: for evaluation
    bool operator==(const ZeroMatrix& rhs) const { return m_rowNum == rhs.m_rowNum && m_colNum == rhs.m_colNum; }
...
public:
    ...
    OneHotVector(const std::size_t colNum, const std::size_t hotPos) : m_colNum{colNum}, m_hotPos(hotPos) {}
