...
int main() {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        if (epoch % 60 == 0 || epoch == epochs - 1) {
            std::cout << "epoch=" << std::setw(3) << epoch << "  mse=" << std::fixed << std::setprecision(6) << loss
                      << '\n';
        }
    }
    ...
    probe.setValue(0, 0, 2.0f);
    probe.setValue(0, 1, 2.0f);
    probe.setValue(1, 0, 6.0f);
    probe.setValue(1, 1, 1.0f);
    probe.setValue(2, 0, 0.0f);
    probe.setValue(2, 1, 3.0f);
    ...
    for (std::size_t i = 0; i < probe.rowNum(); ++i) {
        ...
        std::cout << "x=[" << probe(i, 0) << ", " << probe(i, 1) << "]"
                  << "  pred=" << probe_prediction(i, 0) << "  expected=" << expected << '\n';
    }
